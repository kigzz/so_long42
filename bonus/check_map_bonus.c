...
/*   Updated: 2022/02/04 18:50:27 by bpouchep         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_objects(t_program *game)
{
	...
	if (game->objs.exit == 0 || game->objs.coll == 0
		|| game->objs.player != 1)
		return (1);
	return (0);
}
...
int	check_map(t_program *game)
{
	if (!game->map_save)
		return (1);
	else if (check_rectangular(*game) || check_walls(*game)
		|| check_parameters(*game) || check_objects(game))
	{
